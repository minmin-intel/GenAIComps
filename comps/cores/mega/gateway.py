...
from ..proto.docarray import LLMParams, GeneratedDoc, TextDoc, SearchedDoc
...
class RAGToolGateway(Gateway):
    """
    Very similar to ChatQnAGateway, but not streaming response.
    Instead LLM microservice returns GeneratedDoc
    """
    def __init__(self, megaservice, host="0.0.0.0", port=8889):
        super().__init__(
            megaservice, host, port, str(MegaServiceEndpoint.RAGTOOL), ChatCompletionRequest, ChatCompletionResponse
        )

    async def handle_request(self, request: Request):
        data = await request.json()
        print('request data: ', data)
        stream_opt = False #changed here
        ...
        for node, response in result_dict.items():
            ...
            # Here it suppose the last microservice in the megaservice is LLM.
            if (
                isinstance(response, GeneratedDoc) # changed here
                and node == list(self.megaservice.services.keys())[-1]
